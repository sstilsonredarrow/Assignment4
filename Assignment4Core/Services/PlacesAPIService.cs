...
namespace Assignment4.Services
{
    public class PlacesAPIService : IPlacesAPIService
    {
        ...
        public async Task<List<MapPosition>> GetSearchResults(string search, Position position, string searchType)
        {
            using (var client = new HttpClient())
            {
                ...
                List<MapPosition> placeResults = new List<MapPosition>();

                result.results.ForEach(r => placeResults.Add(new MapPosition(r)));
                return placeResults;
